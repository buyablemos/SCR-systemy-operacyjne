...
void wczytaj_dane_uzytkownikow(const char *sciezka_pliku, Uzytkownik *uzytkownicy, int *liczba_uzytkownikow)
{
    ...
    *liczba_uzytkownikow = 0;
    while (fscanf(plik, "%d %s %s %[^\n]", &uzytkownicy[*liczba_uzytkownikow].id, uzytkownicy[*liczba_uzytkownikow].hash,
                  uzytkownicy[*liczba_uzytkownikow].email, uzytkownicy[*liczba_uzytkownikow].nazwa) == 4 &&
           *liczba_uzytkownikow < MAX_UZYTKOWNIKOW)
    {
        ...
        for (int i = 0; i < liczba_uzytkownikow; i++)
        {
            if (uzytkownicy[i].zlamane == false){
                ...
                {   pthread_mutex_lock(&parametry->mutex_zew);
                    ...
                    printf("Watek:%d Znaleziono hasło: ID=%d, Email=%s, Nazwa użytkownika=%s, Hasło=%s\n", id,
            ...
            for (int i = 0; i < liczba_uzytkownikow; i++)
            {
                if (uzytkownicy[i].zlamane == false)
                {
                    ...
                    {   
                        ...
                        printf("Watek:%d Znaleziono hasło z prefiksem: ID=%d, Email=%s, Nazwa użytkownika=%s, Hasło=%s\n", id,
            ...
            for (int i = 0; i < liczba_uzytkownikow; i++)
            {
                if (uzytkownicy[i].zlamane == false)
                {
                    ...
                    {   pthread_mutex_lock(&parametry->mutex_zew);
                        ...
                        printf("Watek:%d Znaleziono hasło z postfiksem: ID=%d, Email=%s, Nazwa użytkownika=%s, Hasło=%s\n", id,
            ...
            for (int y = 0; y < 100; y++)
            {
                ...
                for (int i = 0; i < liczba_uzytkownikow; i++)
                {
                        ...
                        {   pthread_mutex_lock(&parametry->mutex_zew);
                            ...
                            printf("Watek:%d Znaleziono hasło z prefiksem i postfixem: ID=%d, Email=%s, Nazwa użytkownika=%s, Hasło=%s\n", id,
    ...
    while (1)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                printf("Watek konsumenta ID:%d  wyrzucam z bazy hasło: ID=%d, Email=%s, Nazwa użytkownika=%s, Hasło=%s\n", myid,
                       parametry.uzytkownicy[parametry.haslo_zlamane].id, parametry.uzytkownicy[parametry.haslo_zlamane].email, parametry.uzytkownicy[parametry.haslo_zlamane].nazwa, parametry.uzytkownicy[parametry.haslo_zlamane].hash);
...
int main()
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < MAX_UZYTKOWNIKOW; ++i)
        {
            ...
            memset(uzytkownicy[i].email, 0, sizeof(uzytkownicy[i].email));
            memset(uzytkownicy[i].nazwa, 0, sizeof(uzytkownicy[i].nazwa));
        }
